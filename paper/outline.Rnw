...
to screen a number of different cell types, the concentrations are not necesarily optimal for these PBMC samples. 
...
this does not necesarily imply staining is heterogeneous.

One common goal for a BD FACS CAP screen is to identify markers 
