...
to performance improvements in automated gating. Comparisons of the \Rpackage{plateCore} and FlowJo\texttrademark 
...
The design and execution of large flow experiments, like BD FACS\texttrademark CAP, highlight the 
...
1) Discuss results of this study
2) plateCore is not intended to replace GUI tools, often used in parallel for BD FACS CAP.
	It's difficult to use GUI tools to analyze large datasets, difficult to handle annotation
3) Open platform
	Other 
...
Results from this BD FACS\texttrademark CAP study of PBMC lymphocytes show that a complex analysis of a 96-well plate,
stained with 189 antibodies, can be constructed in 15-20 lines of code with \Rpackage{plateCore}. Using FlowJo\texttrademark
as a benchmark for evaluating \Rpackage{plateCore} gating, the non-parametric method for calculating the
isotype gate (i.e. non-specific binding) closely replicates gates set by expert cytometrists (see Figure~\ref{fig:pcVSman}).
...
The percentage
of positive cells estimated using \Rpackage{plateCore} gating are strongly correlated with the FlowJo\texttrademark
results. 
