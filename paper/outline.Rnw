...
versus the observed (circles) and expected (line) sample standard deviation.}
\label{fig:donorVar}
\end{figure}
...
is initially set using the isotype control well. Data for each plate is imported into FJ using the template and
lymphocytes are selected using a morphology (FSC-SSC) gate. Event data for the isotype well is then
visualized on a log scale, and the expression threshold for each stained channel is set by picking a
value that lies above the bulk of the events. For BD FACS CAP, the isotype gate is set so that less than
...
Automating the creation and modification of isotype gates made by cytometrists in a BD FACS CAP analysis
is extremely difficult. Gates may be adjusted based on expert knowledge about the performance of 
specific antibody types and dyes, or after identifying positive test samples. The automated approach
employed in \Rpackage{plateCore} determines the threshold using only the isotype well.  The gate for each
stained channel is set according to:
\begin{equation}
\text{Isotype Gate} = \max (\text{99th \%tile, MFI + 5 MAD}).
\label{isoGate}
\end{equation}
Where MFI is the Median Fluorescence Intensity and MAD is Median Absolute Deviation of the fluorescence signal
on a linear scale. 
...
Figure~\ref{fig:pcVSman} shows how the results from FlowJo$^{\text{TM}}$ compare to \Rpackage{plateCore}.
