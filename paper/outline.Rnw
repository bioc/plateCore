...
Flow Cytometry High Content Screening (FC-HCS) experiments require a systematic approach to
...
The availability of \Rpacakge{flowCore} and \Rpackage{flowViz} has already enabled the creation of new tools for
quality assessment of large flow experiments (\Rpackage{flowQ}) and model-based clustering (\Rpackage{flowClust}).
\Rpackage{plateCore} also takes advantage of the functionality in \Rpackage{flowCore} and \Rpackage{flowViz}
to work on \Robject{flowPlate} objects that represent large, plate-based flow datasets.  
