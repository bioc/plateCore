...
The 5 PBMC plates were analyzed using the approach shown in Figure~\ref{fig:analysis} and 
results were stored in \Robject{flowPlates}.  These \Robject{flowPlates} contain the layout and
annotation for the plate, morphology gated (FSC-SSC) events, and parameters values for the 
