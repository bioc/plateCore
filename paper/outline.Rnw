...
Analysis of plate-based, Flow Cytometry High Content Screening (FC-HCS) experiments requires a systematic approach to
...
The 
The lack of an open analysis platform represents a significant barrier to developing the new methods required for FC-HCS data processing.  
Statisticans 

Flow cytometry packages available through the Bioconductor \citep{BIOC} project provide an open analysis platform that
...
The basic data processing tools for importing, transforming, gating, and
organizing raw flow cytometry data are in \Rpackage{flowCore} \citep{hahne2009}.
Once the data has been imported into R/Bioconductor platform, \Rpackage{flowViz} 
can be used to create sophisticated visualizations for gating and reporting FC-HCS results.



The ability for statisticians to easily access flow data on a statistical software platform using \Rpackage{flowCore}  
facilitates the development of tools such as , which 
perform automated gating using model-based clustering.  



  The combination
of these packages provides a set of freely available, flexible, and computationally efficient FC-HCS tools.


The \Rpackage{plateCore} package extends the \Rpackage{flowCore} and \Rpackage{flowViz} packages
to work on \Robject{flowPlate} objects that represent large flow datasets.  An example of the progression from raw FCM data files to a completed \Rpackage{plateCore} analysis is shown in Figure~\ref{fig:analysis}.
...
Output from the \Rpackage{plateCore} analysis is stored in \Robject{flowPlate} objects, which
contain the gated FCS events along with the isotype gating results. The event level can
be visualized 

The event level data can be 
visualized using plotting functions from \Rpackage{plateCore} and \Rpackage{flowViz}. Additionally,
the \Robject{wellAnnotation} table associated with each plate can be exported, merged, and 
analyzed using microarray tools in R (Figure~\ref{fig:pbmcHeat}).


Analysis output for each 96-well plate was stored in a \Robject{flowPlate}, which are the \Rpackage{plateCore}
data structures containing the raw FCM input and isotype gating results.
The \Robject{wellAnnotation} for each \Robject{flowPlate} was then merged
into a single \Robject{data.frame}, containing the percent positive results from each plate.
These values can then visualized
using microarray tools in R (). 
