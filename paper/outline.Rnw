...
\subsection*{Methods}
\Rpackage{plateCore} was used to analyze the data from a BD FACS\texttrademark CAP screening experiment where 5
...
quality assessment of large flow experiments, such as \Rpackage{flowQ} \citep{}, and model-based clustering and automated
gating, such as \Rpackage{flowClust} \cite{lo2008}.
...
BD FACS\texttrademark CAP was designed as a cell characterization tool to screen for the presence a large number of different human 
cell surface markers. In this study, PBMC samples (previously frozen) from two donors were analyzed on a BD FACScalibur
using BD FACS\texttrademark CAP staining. The analysis was performed on 96-well plates with
...
The complete list of BD FACS\texttrademark CAP antibodies can be found at http://www.bd.com/technologies/discovery\_platform/BD\_FACS\_CAP.asp. 
...
Positive for BD FACS\texttrademark CAP was defined as having more than 10\% of
events above the isotype gate. Since antibody concentrations used in BD FACS\texttrademark CAP are designed
...
Since the majority of antibodies on the BD FACS\texttrademark CAP staining plate are known to bind different
...
mfiDf <- read.csv('mfiDf.csv',stringsAsFactors=FALSE)
plot(mfiDf$MFI.Ratio,mfiDf$PP, log="x",xlab="MFI Ratio",ylab="Percent Positive")
@
...
using FlowJo\texttrademark is extremely challenging. Cytometrists adjust gates based on expert knowledge about the performance of 
...
Looking in detail at one case where the \Rpackage{plateCore} and FlowJo\texttrademark methods disagree, such
as CD98 on plate 9208 (Figure~\ref{fig:disagree}), highlights situations where our automated gating
approach will fail. In this case the marker is positive (86\% by FlowJo\texttrademark) but is 
...
%that have been assayed using BD FACS\textrademark CAP (results not shown). Within a particular cell type we 
...
print(xyplot(FJ ~ R | as.factor(plate), fjVSrDf,xlab="plateCore",ylab="FlowJo", aspect="iso"))
@
\caption{Percent positive results for 189 BD FACS\textrademark CAP markers analyzed using either \Rpackage{plateCore}
or FlowJo\texttrademark for the 5 PBMC plates. Markers that varied the most between the the two methods tended to have
intermediate percent positive values (30\% to 70\%), reflecting the difficulty of gating populations whose MFI is 
near the isotype threshold. Additionally, since BD FACS\textrademark CAP was designed as screening tool to 
...
\caption{Dotplot for CD98 in well G01 and its isotype control well H08 for plate 9208. FlowJo\texttrademark
