...
allowing analysis pipelines to match the high-throughput capacity of modern cytometers. Current approaches to
FC-HCS analysis semi-automated at best, often requiring significant manual intervention to identify cells of interest and
set the appropriate gates. 



Manual making
it difficult to develop standards and reproduce analyses. 

A lack of tools for managing the raw data and for providing access to the
results also inhibits collaborative develop of . 




Flow cytometry packages available through Bioconductor provide an open analysis platform that
...
organizing raw flow cytometry data. \Rpackage{flowViz} provides sophisticated visualizations based on Trellis displays. 
\Rpackage{flowClust} implements model-based clustering approaches for automated gating. 
