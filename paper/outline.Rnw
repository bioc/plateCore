...
deployed. \Rpackage{plateCore} is a R/Bioconductor packaged created to make processing and analysis of large, complex flow cytometry (FCM) datasets
easier. 
...
deal with the large volume information. Similarly, the analysis presented in this study is a joint effort to validate \Rpackage{plateCore} as a 
FC-HCS analysis tool by comparing the results from an expert manual analysis to the output from \Rpackage{plateCore}. 
...
using BD FACS\texttrademark CAP (ref). On each plate, there are 189 different human cell surface antibody-dye conjugates along
with the appropriate isotype controls. Test antibodies and isotypes are arrayed 3 per well, and the data was compensated
