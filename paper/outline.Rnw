...
enable automated processing. The \Rpackage{flowCore} \citep{hahne2009} package contains the framework for importing, transforming, gating, and
organizing raw flow cytometry data. \Rpackage{flowViz} \citep{sarkar2008ufv} supports sophisticated visualizations based on Trellis displays. 
\Rpackage{flowClust} \citep{lo2008} implements model-based clustering approaches for automated gating. \Rpackage{plateCore}
