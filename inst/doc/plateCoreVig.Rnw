...
\Rpackage{flowStats}, and \Rpackage{flowClust}. Once the layout of the experiment has been standardized, then getting
...
The \Robject{pbmcPlate} is actually one of the 5 plates that make up the Peripheral Blood Mononucleocyte Cell (PMBC) dataset in the example analysis section. 
...
are modeled on the \Rclass{AffyBatch} structure used for micorarray analysis in Bioconductor.  The \Robject{pbmcPlate} is comprised of an
...
describe the contents of a well. The wellAnnotation format is easier for humans to create and maintain, and \Rpackage{plateCore}
...
cells in only the channel interest, instead of having two dimensional gate that includes the forward scatter (FSC) channel. The background
...
correction on lymphocytes is subtle, but it can have dramatic effects for other cells types that have a wider range of FSC values.
...
showing both the test and negative control samples on the same graph can help to set the appropriate gate. These plots are 
also made using xyplot. An example of the code use to generate the overlay plots in Figure ~\ref{fig:isoGate2}is shown below.
<<eval=FALSE>>=
...
control well (MFI.Ratio). In this PMBC example a number of the samples are heterogenous, so the MFI and MFI.Ratio
...
%on the cytometer so there is no need to correct for spillover.
%
%The data for each plate was analyzed exactly as shown in the previous sections. The default dataset that comes with \Rpackage{plateCore}
%is one of these five plates, and the same gating approach works for the other 4 plates. The results of the analysis are summarized
%in Figure ~\ref{fig:pbmcHeat}, which shows the percentage of postive cells for each antibody on each plate. Figure ~\ref{fig:pbmcHeat}
%also shows that there are several markers which are differentially expressed between the different PBMC samples. The dotplots
%for one of these differentially expressed markers, CDbd69, is shown in Figure ~\ref{fig:pbmcCDbd69}. The plots in 
%Figure ~\ref{fig:pbmcCDbd69} were created by making a virtual plate using \Rfunction{fpbind}, which binds
%\Rclass{flowPlate} objects together. 
%<<eval=FALSE>>=
%virtPlate <- fpbind(plates[[1]],plates[[2]],plates[[3]],plates[[4]],plates[[5]])
%xyplot(`FL2-H` ~ `FSC-H` | as.factor(Ab.Name) * as.factor(plateName), 
%  transform("FL2-H"=log10) %on% virtPlate,smooth=FALSE,
%  displayFilter=TRUE,filter="Negative.Control")
%@
%
%\begin{figure}
%\begin{center}
...
%
%temp <- read.delim("../pbmcReports/ppExp.csv",stringsAsFactors=FALSE,header=TRUE)
%rownames(temp) <- temp[,1]
%temp <- temp[,-1]
%
%mapcol <- rev(heat.colors(10))
%b <- seq(0,100,by=10)
%heatmap.2(as.matrix(temp),col=mapcol,breaks=b,scale='none',trace='none',cexRow=0.6)
%
%@
%\end{center}
%\caption{Heatmap showing the percentage of positive cells for the 5 different PBMC lymphocyte plates. Only markers with
%more than 10\% positive cells in at least one sample are shown on the plot.}
%\label{fig:pbmcHeat}
%\end{figure}
%
%
...
%
%
%print(xyplot(`FL2-H` ~ `FSC-H` | as.factor(FL2.H) * as.factor(plateName), transform("FL2-H"=log10) %on% virtPlate,smooth=FALSE,
%				displayFilter=TRUE,filter="Negative.Control"))
%
%@
%\end{center}
%\caption{Dotplots for CDbd69, which was differentially expressed between the 5 PBMC plates.}
